...
// Custom parser component to beautifully format Kai's markdown replies without exposing ugly system asterisks (**) or hashes (##).
function parseBoldText(text: string) {
  const parts = text.split(/\*\*([^*]+)\*\*/g);
  return parts.map((part, i) => {
    if (i % 2 === 1) {
      return (
        <strong key={i} className="font-extrabold text-indigo-950 bg-indigo-50/70 px-1.2 py-0.5 rounded text-[11px] border border-indigo-100/30">
          {part}
        </strong>
      );
    }
    ...
    return subParts.map((sub, j) => {
      if (j % 2 === 1) {
        return <em key={j} className="italic text-indigo-900 font-medium">{sub}</em>;
      }
      return sub;
    });
  });
    ...
    <div className="space-y-1.5 text-xs text-slate-750 leading-relaxed font-normal">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith('###') || trimmed.startsWith('##')) {
          ...
          return (
            <h5 key={idx} className="font-extrabold text-[12px] text-indigo-950 mt-3 mb-1 tracking-tight flex items-center gap-1">
              <span className="w-1 h-3 rounded bg-indigo-500 inline-block"></span>
        ...
        if (trimmed.startsWith('*') || trimmed.startsWith('-')) {
          ...
          return (
            <div key={idx} className="flex gap-1.5 pl-2 items-start my-1 text-slate-700">
              <span className="text-indigo-500 font-bold text-[13px] leading-3">•</span>
      ...
      const context = {
        ...
        itineraryText: inferenceResult?.itinerary 
          ? inferenceResult.itinerary.map(day => (
              ...
              `🍽️ Food: ${day.food?.name || 'Local culinary guide'}\n` +
              `🏛️ Attractions: ${day.pois.map(poi => `${poi.name} [Intensity: ${poi.activity_intensity}, Budget: ${poi.costTier}]`).join(', ')}`
            )).join('\n\n') 
